import React from "react";
import { List } from "semantic-ui-react";

const HyruleField = () => {
    return (
        <List divided relaxed>
            <List.Item>
                <List.Icon name="check" size="large" verticalAlign="middle" />
                <List.Content>
                    <List.Header>Ocarina of Time</List.Header>
                </List.Content>
            </List.Item>
            <List.Item>
                <List.Icon name="check" size="large" verticalAlign="middle" />
                <List.Content>
                    <List.Header>Song of Time</List.Header>
                </List.Content>
            </List.Item>
            <List.Item>
                <List.Icon name="check" size="large" verticalAlign="middle" />
                <List.Content>
                    <List.Header>Near Kakariko Grotto</List.Header>
                </List.Content>
                <List.List>
                    <List.Item>
                        <List.Icon name="check" size="large" verticalAlign="middle" />
                        <List.Content>
                            <List.Header>Hyrule Field Near Kakariko Skulltula</List.Header>
                        </List.Content>
                    </List.Item>
                </List.List>
            </List.Item>
            <List.Item>
                <List.Icon name="check" size="large" verticalAlign="middle" />
                <List.Content>
                    <List.Header>Inside Fence Grotto</List.Header>
                </List.Content>
                <List.List>
                    <List.Item>
                        <List.Icon name="check" size="large" verticalAlign="middle" />
                        <List.Content>
                            <List.Header>Hyrule Field Deku Salesman Grotto</List.Header>
                        </List.Content>
                    </List.Item>
                </List.List>
            </List.Item>
            <List.Item>
                <List.Icon name="check" size="large" verticalAlign="middle" />
                <List.Content>
                    <List.Header>Field Open Grotto</List.Header>
                </List.Content>
                <List.List>
                    <List.Item>
                        <List.Icon name="check" size="large" verticalAlign="middle" />
                        <List.Content>
                            <List.Header>Hyrule Field South Grotto</List.Header>
                        </List.Content>
                    </List.Item>
                </List.List>
            </List.Item>
            <List.Item>
                <List.Icon name="check" size="large" verticalAlign="middle" />
                <List.Content>
                    <List.Header>Field Diving Grotto</List.Header>
                </List.Content>
                <List.List>
                    <List.Item>
                        <List.Icon name="check" size="large" verticalAlign="middle" />
                        <List.Content>
                            <List.Header>Diving Heart Piece Grotto</List.Header>
                        </List.Content>
                    </List.Item>
                </List.List>
            </List.Item>
            <List.Item>
                <List.Icon name="check" size="large" verticalAlign="middle" />
                <List.Content>
                    <List.Header>Field Cow Grotto</List.Header>
                </List.Content>
                <List.List>
                    <List.Item>
                        <List.Icon name="check" size="large" verticalAlign="middle" />
                        <List.Content>
                            <List.Header>Hyrule Field Near Gerudo Valley Skulltula</List.Header>
                        </List.Content>
                    </List.Item>
                </List.List>
            </List.Item>
            <List.Item>
                <List.Icon name="check" size="large" verticalAlign="middle" />
                <List.Content>
                    <List.Header>Near Market Grotto</List.Header>
                </List.Content>
                <List.List>
                    <List.Item>
                        <List.Icon name="check" size="large" verticalAlign="middle" />
                        <List.Content>
                            <List.Header>Hyrule Field North Grotto</List.Header>
                        </List.Content>
                    </List.Item>
                </List.List>
            </List.Item>
            <List.Item>
                <List.Icon name="check" size="large" verticalAlign="middle" />
                <List.Content>
                    <List.Header>Field Southeast Grotto</List.Header>
                </List.Content>
                <List.List>
                    <List.Item>
                        <List.Icon name="check" size="large" verticalAlign="middle" />
                        <List.Content>
                            <List.Header>Hyrule Field Forest Grotto</List.Header>
                        </List.Content>
                    </List.Item>
                </List.List>
            </List.Item>
        </List>
    );
}

export default HyruleField;